...
private:
    const std::vector<ResidentialComponent*>& buildings;  // Reference to buildings in a ResidentialComplex
    size_t currentIndex;                      // Current index in the iteration

public:
    // Constructor: initialize with a vector of Building pointers
    LinearBuildingIterator(const std::vector<ResidentialComponent*>& buildings)
    ...
    ResidentialComponent* next() override {
        if (!hasNext()) {
