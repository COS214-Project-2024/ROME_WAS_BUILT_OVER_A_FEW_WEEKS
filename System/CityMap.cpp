...
void CityMap::removeWarehouse(Warehouse *originator){
    // call removeBuilding to adjust the traffic
    this->cityHall->decreaseNumIndustrialBuildings();
    removeBuilding(originator);
    this->cityHall->decreaseStorageCapacity();
}

void CityMap::removeFactory(Factory *originator){
    // call removeBuilding to adjust the traffic
    this->cityHall->decreaseNumIndustrialBuildings();
    this->cityHall->decreaseNumFactories();
    removeBuilding(originator);
}
...
void CityMap::generateResources(Factory *factory){
    ...
    if(type.find('Steel') == std::string::npos){
        //DO NOTHING
    }else{
